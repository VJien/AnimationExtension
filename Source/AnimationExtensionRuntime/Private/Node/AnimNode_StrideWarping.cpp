...
void FAnimNode_StrideWarping::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (auto& FootData : FeetData)
	{
		...
		NEW_MODIFY
		FootData.FKBoneTransform = Output.Pose.GetComponentSpaceTransform(FootData.FKFootBoneIndex);
#if ENABLE_ANIM_DEBUG
		if (bShowDebug)
		{
			...
			MyAnimInstanceProxy->AnimDrawDebugSphere(FootWorldLocation, 2.f, 4, FColor::Cyan);
		}
#endif
	}
	...
	for (auto& FootData : FeetData)
	{
		...
		NEW_MODIFY
		const FVector FKFootLocation = FootData.FKBoneTransform.GetLocation(); 

		const FVector HipBoneLocation = Output.Pose.GetComponentSpaceTransform(FootData.HipBoneIndex).GetLocation();

		FVector StrideWarpingPlaneOrigin = (FMath::Abs(GravityDir | FloorPlaneNormal) > DELTA) ? FMath::LinePlaneIntersection(HipBoneLocation, HipBoneLocation + GravityDir, FKFootLocation, FloorPlaneNormal) : IKFootLocation;

		const FVector ScaleOrigin = FVector::PointPlaneProject(FKFootLocation, StrideWarpingPlaneOrigin, StrideWarpingPlaneNormal);



		/*	FVector WarpedLocation = StrideWarpingPlaneOrigin;
			WarpedLocation.Y = ScaleOrigin.Y + (FKFootLocation- ScaleOrigin).Y * ActualStrideScaling;
			WarpedLocation.Z = FKFootLocation.Z;
			WarpedLocation.X = FKFootLocation.X;*/


		FVector WarpedLocation = ScaleOrigin + (FKFootLocation - ScaleOrigin) * ActualStrideScaling;



		FootData.IKBoneTransform.SetLocation(WarpedLocation);
		FootData.IKBoneTransform.SetRotation(FootData.FKBoneTransform.GetRotation());
#if ENABLE_ANIM_DEBUG
		if (bShowDebug)
		{
			FVector FootWorldLocation = MyAnimInstanceProxy->GetComponentTransform().TransformPosition(FootData.IKBoneTransform.GetLocation());
	...
	if ((PelvisBoneIndex != INDEX_NONE) && (PelvisPostAdjustmentAlpha > 0.f))
	{
		...
		//const FVector SmoothAdjustedPelvisLocation = InitialPelvisLocation + FMath::Lerp(FVector::ZeroVector, TargetAdjustment, PelvisPostAdjustmentAlpha);
...
bool FAnimNode_StrideWarping::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	...
	bool a = (PelvisBone.GetCompactPoseIndex(RequiredBones) != INDEX_NONE);
	bool b = (IKFootRootBone.GetCompactPoseIndex(RequiredBones) != INDEX_NONE);

	float d = StrideScalingScaleBiasClamp.ApplyTo(StrideScaling, 0.f);
	bool c = (!FMath::IsNearlyEqual(d, 1.f, 0.001f) || PelvisAdjustmentInterp.IsInMotion());
